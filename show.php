...
$query="select sc.id cid,sc.module_id,sc.title,sc.content,sc.time,sc.member_id,sc.times,sm.name,sm.photo from cfc_content sc,cfc_member sm where sc.id={$_GET['id']} and sc.member_id=sm.id";
$result_content=execute($link,$query);
if(mysqli_num_rows($result_content)!=1){
	skip('index.php', 'error', '本帖子不存在!');
}
$query="update cfc_content set times=times+1 where id={$_GET['id']}";
execute($link,$query);
...
$query="select * from cfc_son_module where id={$data_content['module_id']}";
$result_son=execute($link,$query);
$data_son=mysqli_fetch_assoc($result_son);

$query="select * from cfc_father_module where id={$data_son['father_module_id']}";
$result_father=execute($link,$query);
$data_father=mysqli_fetch_assoc($result_father);

$query="select count(*) from cfc_reply where content_id={$_GET['id']}";
...
<div id="main" class="auto">
	...
	<?php }?>
	<?php 
	$query="select sm.name,sr.member_id,sr.quote_id,sm.photo,sr.time,sr.id,sr.content from cfc_reply sr,cfc_member sm where sr.member_id=sm.id and sr.content_id={$_GET['id']} order by id asc {$page['limit']}";
	...
	<div class="wrapContent">
		...
		<div class="right">
			...
			<div class="content">
				...
				$query="select count(*) from cfc_reply where content_id={$_GET['id']} and id<={$data_reply['quote_id']}";
				$floor=num($link,$query);
				$query="select cfc_reply.content,cfc_member.name from cfc_reply,cfc_member where cfc_reply.id={$data_reply['quote_id']} and cfc_reply.content_id={$_GET['id']} and cfc_reply.member_id=cfc_member.id";
