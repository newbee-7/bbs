...
$query="select * from cfc_member where id={$_GET['id']}";
...
$query="select count(*) from cfc_content where member_id={$_GET['id']}";
...
<div id="main" class="auto">
	<div id="left">
		<ul class="postsList">
			...
			$query="select
			cfc_content.title,cfc_content.id,cfc_content.time,cfc_content.member_id,cfc_content.times,cfc_member.name,cfc_member.photo
			from cfc_content,cfc_member where
			cfc_content.member_id={$_GET['id']} and
			cfc_content.member_id=cfc_member.id order by id desc {$page['limit']}";
