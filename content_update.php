...
$query="select * from cfc_content where id={$_GET['id']}";
...
if(mysqli_num_rows($result_content)==1){
	...
	if(check_user($member_id,$data_content['member_id'],$is_manage_login)){
		if(isset($_POST['submit'])){
			...
			$query="update cfc_content set module_id={$_POST['module_id']},title='{$_POST['title']}',content='{$_POST['content']}' where id={$_GET['id']}";
			execute($link, $query);
...
<div id="publish">
	<form method="post">
		<select name="module_id">
			...
			<?php 
			$query="select * from cfc_father_module order by sort desc";
			$result_father=execute($link, $query);
			while ($data_father=mysqli_fetch_assoc($result_father)){
				echo "<optgroup label='{$data_father['module_name']}'>";
				$query="select * from cfc_son_module where father_module_id={$data_father['id']} order by sort desc";
