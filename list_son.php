...
$query="select * from cfc_son_module where id={$_GET['id']}";
...
$query="select count(*) from cfc_content where module_id={$_GET['id']}";
$count_all=num($link,$query);
$query="select count(*) from cfc_content where module_id={$_GET['id']} and time>CURDATE()";
$count_today=num($link,$query);

$query="select * from cfc_member where id={$data_son['member_id']}";
...
<div id="main" class="auto">
	<div id="left">
		...
		<ul class="postsList">
			<?php 
			$query="select
			cfc_content.title,cfc_content.id,cfc_content.time,cfc_content.times,cfc_content.member_id,cfc_member.name,cfc_member.photo
			from cfc_content,cfc_member where
			cfc_content.module_id={$_GET['id']} and
			cfc_content.member_id=cfc_member.id
			{$page['limit']}";
			$result_content=execute($link,$query);
			while($data_content=mysqli_fetch_assoc($result_content)){
			$data_content['title']=htmlspecialchars($data_content['title']);
			$query="select time from cfc_reply where content_id={$data_content['id']} order by id desc limit 1";
			...
			$query="select count(*) from cfc_reply where content_id={$data_content['id']}";
			?>
			<li>
				<div class="smallPic">
	...
	<div id="right">
		<div class="classList">
			...
			<ul class="listWrap">
				<?php 
				$query="select * from cfc_father_module";
				$result_father=execute($link, $query);
				...
				<li>
					...
					<ul>
						<?php 
						$query="select * from cfc_son_module where father_module_id={$data_father['id']}";
						$result_son=execute($link, $query);
